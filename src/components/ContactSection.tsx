...
export const ContactSection: React.FC<ContactSectionProps> = ({
  ...
  useEffect(() => {
    ...
    const loadAvailableSlots = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/calendar/available-slots`, {
          cache: 'no-store',
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`${API_URL}/api/inquiries/${submissionReceipt.submissionId}/status`);
        if (res.ok) {
  ...
  const handleSimulateApproval = async () => {
    if (!submissionReceipt?.submissionId || isApprovingDemo) return;

    setIsApprovingDemo(true);
    setFormErrors((prev) => ({ ...prev, server: '' }));

    try {
      const res = await fetch(`${API_URL}/api/inquiries/${submissionReceipt.submissionId}/confirm`, {
        method: 'POST',
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.message || 'Demo approval failed.');
      }
      ...
      setSubmissionReceipt((prev) => prev ? {
        ...
        status: 'confirmed',
        meetLink: data.meetLink || prev.meetLink,
        googleCalendarUrl: data.googleCalendarUrl || prev.googleCalendarUrl,
        outlookCalendarUrl: data.outlookCalendarUrl || prev.outlookCalendarUrl,
      } : null);
    } catch (error) {
      console.error('[DEMO APPROVAL ERROR]', error);
      setFormErrors((prev) => ({
        ...prev,
        server: error instanceof Error ? error.message : 'Demo approval failed. Please try again.',
      }));
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmissionReceipt({
        ...
        googleCalendarUrl: data.googleCalendarUrl,
        outlookCalendarUrl: data.outlookCalendarUrl,
        icsDownloadUrl: `${API_URL}/api/inquiries/${data.submissionId}/calendar.ics`,
      });
      setSubmitted(true);
    } catch (err) {
  ...
  const handleAiInstantAssist = async (field: 'subject' | 'message') => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/ai/instant-assist`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: promptText,
          ...
          <div className="lg:col-span-7">
            ...
            {submitted ? (
              <div className="bg-[#FFFCF8] rounded-2xl p-8 border border-[#C97872]/40 shadow-lg space-y-6 transition-all duration-300 hover:shadow-xl">
                ...
                {submissionReceipt?.status === 'pending_approval' && (
                  <div className="rounded-2xl border border-[#C97872]/40 bg-[#F8F5F0] p-5 space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <div>
                        <p className="text-[10px] font-mono font-bold tracking-[0.18em] text-[#C97872]">DEMO APPROVAL</p>
                        <p className="mt-1 text-xs text-[#706B65]">Test the confirmation flow immediately.</p>
                      </div>
                      <button
                        type="button"
                        onClick={handleSimulateApproval}
                        disabled={isApprovingDemo}
                        className="rounded-xl bg-[#C97872] px-4 py-2.5 text-xs font-bold text-white transition-all hover:bg-[#B06A64] disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {isApprovingDemo ? 'Approving…' : 'Demo: Approve Request'}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="p-5 bg-[#FFFCF8] border border-[#C97872]/50 rounded-2xl space-y-4 text-left text-xs">
                    ...
                    <div className="pt-2 space-y-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        ...
                        <a
                          href={`${API_URL}/api/inquiries/${submissionReceipt?.submissionId}/calendar.ics`}
